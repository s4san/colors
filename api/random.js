...
module.exports = (req, res) => {
  try {
      ...
      if (!Number.isNaN(num) && Number.isFinite(num) && num < 50 && num > 1) {
        ...
        let innerIndex = getRandomWithin(5);
        let colors = outerIndices.map(index => ALL_COLORS[index][innerIndex]);
        res.json({
            colors,
        });
      } else {
          throw new Error('Invalid number, please choose between 1 and 50');
      }
  } catch (e) {
      res.status(500);
      res.json({
